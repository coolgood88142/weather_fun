  ...
  methods: {
    getTaiwanMap: function getTaiwanMap() {
      ...
      return _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee() {
        var width, height, mercatorScale, w, path, svg, url;
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                width = _this.$refs.map.offsetWidth.toFixed(), height = _this.$refs.map.offsetHeight.toFixed(); // 判斷螢幕寬度，給不同放大值
                ...
                _context.next = 5;
                return d3.geo.path().projection( // !important 座標變換函式
                d3.geo.mercator().center([121, 24]).scale(mercatorScale).translate([width / 2, height / 2.5]));

              case 5:
                path = _context.sent;
                _context.next = 8;
                return d3.select('#svg').attr('width', width).attr('height', height).attr('viewBox', "0 0 ".concat(width, " ").concat(height));

              case 8:
                svg = _context.sent;
                // 讓d3抓GeoJSON檔，並寫入path的路徑
                url = 'dist/taiwan.geojson';
                _context.next = 12;
                return d3.json(url, function (error, geometry) {
